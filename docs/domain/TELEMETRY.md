...
Optional collectors degrade explicitly. Wi-Fi and camera absence or failure is explicitly reported como `UNAVAILABLE`, `DISABLED`, `DEGRADED` ou `FAILED`, conforme a causa; playback continues, assim como scheduling, execução offline, Health e sincronização dos outros fatos. Informação ausente não vira zero, sucesso presumido ou valor inventado.
