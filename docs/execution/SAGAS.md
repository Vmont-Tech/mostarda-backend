...
| Liquidação | Scheduler abre ciclo → seleciona Evidence válida/ancorada → calcula shares → autoriza → publica direitos → Financial Platform credita Partner Ledger. | Retry por `SplitShare`/crédito e dedupe por parcela. | Share bloqueada/unclaimed não bloqueia demais; chargeback usa lançamentos financeiros compensatórios. |
| Saque de parceiro | Wallet deriva crédito → parceiro solicita → política aprova → batch → Asaas transfere → Financial Platform reconcilia. | Retry por Withdrawal/tentativa e webhook deduplicado. | Falha libera reserva ou agenda retry; nunca altera Settlement/SplitShare. |
