...
export function createDailyScheduleContentServer(options: DailyScheduleContentServerOptions): Server {
  ...
  return createServer((request, response) => {
    void handleRequest(options, emittedPlaybackKeys, request, response).catch(() => {
...
async function handleRequest(options: DailyScheduleContentServerOptions, emittedPlaybackKeys: Set<string>, request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  if (method === "GET" && url.pathname === "/schedule/current") {
    ...
    const slot = currentDailySlot(options.schedule, now);
    const seconds = Number(new Intl.DateTimeFormat("en-US", {
      ...(options.schedule.timezone === "local" ? {} : { timeZone: options.schedule.timezone }),
      second: "2-digit",
      hourCycle: "h23",
    ...
    const localDate = new Intl.DateTimeFormat("en-CA", { ...(options.schedule.timezone === "local" ? {} : { timeZone: options.schedule.timezone }), year: "numeric", month: "2-digit", day: "2-digit" }).format(now);
    return sendJson(response, 200, { ...slot, playbackKey: `${localDate}:${slot.slotIndex}`, assetUrl: `/assets/${encodeURIComponent(slot.content.assetId)}`, nextSlotInMs, slotEndsAt: now.getTime() + nextSlotInMs });
  }
  ...
  if (method === "POST" && url.pathname.startsWith("/playback/")) {
    ...
    const slot = options.schedule.slots.find((candidate) => candidate.slotId === slotId);
    ...
    if (emittedPlaybackKeys.has(playbackKey)) return sendJson(response, 200, { accepted: true, deduplicated: true, slotId, playbackKey });
    emittedPlaybackKeys.add(playbackKey);
